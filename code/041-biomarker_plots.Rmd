...
analyte_names <- colnames(avg_data)[9:50] 
...
for (analyte in analyte_names) {
  p <- ggplot(avg_data, aes_string(x = analyte, y = avg_sod)) +
    geom_point() +
    geom_smooth(method = "lm", se = TRUE, color = "blue") +
    labs(
      title = paste(analyte, "vs Biomarker Activity"),
  ...
  # Save the plot
  ggsave(filename = paste0("analyte_plots/", analyte, "_vs_Biomarker.png"), plot = p, width = 6, height = 4)
}
