...
# plot with shapes
ggplot(filtered_data, aes(x = reorder(latlong_group, as.numeric(latlong_group)), y = p450)) +
  geom_boxplot(outlier.shape = NA, color = "gray50") + # Keep boxplot simple
  geom_jitter(aes(shape = latlong_group), width = 0.2, size = 2) + # Add points with shapes
  theme_minimal() +
  labs(
  ...
  scale_shape_manual(
    values = seq(0, length(unique(filtered_data$latlong_group)) - 1), # Use unique shapes
...
# identify and remove outliers using this ChatGPT function
iqr_threshold <- function(x) {
  Q1 <- quantile(x, 0.25, na.rm = TRUE)
  Q3 <- quantile(x, 0.75, na.rm = TRUE)
  IQR <- Q3 - Q1
...
# create latitude and longitude groups from p450 data to ensure groupings for both biomarkers are the same.
# this next step is a repeat of the steps from p450 in case I don't run the p450 first. Note that I need to load the p450 data with the sod data so this doesn't throw an error.

set.seed(123)
clusters <- kmeans(sdata[, c("latitude", "longitude")], centers = 5)
sdata <- sdata %>%
  mutate(latlong_group = as.factor(clusters$cluster))

# save p450 clustering 
#p450_clusters <- clusters

# lat/long ranges for each group (this will be overriden by the p450 clusters, but I don't know how I got this screwed up and have to go back and fix this step to make sure i'm not creating a different problem.)
...
# DO NOT RUN THIS SECTION AGAIN UNTIL I FIX THE CLUSTERS
# assign the p450 clusters to the SOD data
sdata <- sdata %>%
  mutate(
...
# REDUNDANT FROM THE OUTLIER CULLING ABOVE
# remove outliers 
