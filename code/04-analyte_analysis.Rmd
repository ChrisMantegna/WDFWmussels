...
for (group in groups) {
  ...
  group_means <- data %>%
    group_by(.data[[group]]) %>%
    summarise(across(all_of(c(metrics_of_interest, analyte_columns)), mean, na.rm = TRUE), .groups = "drop")
  ...
  for (metric in metrics_of_interest) {
    ...
    for (analyte in analyte_columns) {
      ...
      # Ensure both columns exist in the grouped dataframe
...
# Print results
print(group_spearman_results)

# Save results
write.csv(group_spearman_results, "/Users/cmantegna/Documents/GitHub/WDFWmussels/output/group_spearman_results.csv", row.names = FALSE)

# Filter significant correlations
significant_group_correlations <- group_spearman_results %>% filter(P_Value < 0.05)
print(significant_group_correlations)
