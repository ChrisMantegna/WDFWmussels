...
Pindv<- read.csv("/Users/cmantegna/Documents/WDFWmussels/data/avgP450_analytes.csv")
...
# p450
## dumbbell - this one doesn't work correctly.
```{r}
...
# Sorting the data by reporting area and site_name
data <- data %>% arrange(reporting_area, site_name)

ggplot(data, aes(x = p450, y = reorder(site_name, p450))) +
  geom_point(aes(color = factor(reporting_area)), size = 3) +
  geom_dumbbell(aes(xend = p450), 
                size = 1.5, color = "gray", 
                point.colour.l = "blue", 
                point.colour.r = "red") +
  labs(
    x = "p450 Values", 
    y = "Site Name", 
    title = "p450 Values by Site Grouped by Reporting Area"
  ) +
  theme_minimal() +
...
ggplot(site_p450_range, aes(y = reorder(site_name, site_name))) +  # Ordering alphabetically by site_name
