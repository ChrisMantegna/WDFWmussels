...
# Asking: Are there correlations between the contaminants and the metrics(Spearman)? Is there a relationship between the contaminants and metrics that could indicate dependence or independence (Kendall's) in the Spearman results?
## Spearman & Kendall's
### metrics & all indices
```{r}
...
# FYI: these aren't used in the loop but fine to keep listed
id_columns <- c("site_name", "site_number", "reporting_area", "latitude", "longitude")

analyte_vars <- c("chlordane_index", "ddt_index", "hch_index", "heavy_metal_index",
                  "nutrient_metal_index", "total_metal_index",
                  "pah_lmw_index", "pah_hmw_index", "pah_add_index", "total_pah_index",
                  "pbde_index", "pcb_index", "pesticide_indexe")  
...
for (metric in metrics_vars) {
  ...
  for (analyte in analyte_vars) {
    ...
    # need enough paired values & some variation
    if (length(x_valid) < 4L ||
        length(unique(x_valid)) < 2L ||
        length(unique(y_valid)) < 2L) next
...
view(corr_full)  # capital V
#write.csv(corr_full, "../output/tables/spearman_kendall_results/correlation_all_indices.csv", row.names = FALSE)
