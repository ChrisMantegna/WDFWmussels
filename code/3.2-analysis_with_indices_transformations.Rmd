---
title: "3.2- Analysis with transformation and indices"
output:
  pdf_document: 
    fig_width: 20
    fig_height: 9
...
site_data<- read.csv("../data/cleaned/site_level_complete_indices_10012025.csv")
sample_data<- read.csv("../data/interim_df/sample_level_scaling_09292025.csv")
sample_ibr<- read.csv("../data/interim_df/sample_level_ibr_09292025.csv")
...
# Test Normality
## Determines what stats I can run
```{r}
...
# save
#write.csv(shapiro_results, "../output/tables/shapiro_scaled_and_indices_10012025.csv", row.names = FALSE)

```

# Normally distributed indices - ANOVA + Tukey's Post hoc
## Are there differences between these values at the site and reporting area level?
```{r}

#p450 - site level for RA and sample level (tc value) for the site
...
# these may not be consequential in what I need
#length
...
```
## Tukey's for p450 and ibr_morph
```{r}
...
view(tukey_table_with_cld)
#write.csv(tukey_table_with_cld, "../output/tables/tukey_p450_ra_10012025.csv", row.names = FALSE) 

# ibr_morph, site
# the object you're separating on; e.g., tukey_df
names(tuk)
dplyr::glimpse(tuk)
head(tuk)
...
view(tukey_table_with_cld)
#write.csv(tukey_table_with_cld, "../output/tukey_morph_site_10022025.csv", row.names = FALSE) 

# ibr_morphometrics - reporting area
...
view(tukey_table_with_cld)
#write.csv(tukey_table_with_cld, "../output/tables/tukey_ibr_morph_ra_10012025.csv", row.names = FALSE) 

```

## Tukey's for length, width, height, initial and final weight - all site, no RA
```{r}

# length, site - not significant
...
view(tukey_table_with_cld)
#write.csv(tukey_table_with_cld, "../output/tukey_length_site_10012025.csv", row.names = FALSE) 
...
view(tukey_table_with_cld)
#write.csv(tukey_table_with_cld, "../output/tukey_width_site_10012025.csv", row.names = FALSE) 
...
view(tukey_table_with_cld)
#write.csv(tukey_table_with_cld, "../output/tukey_height_site_10022025.csv", row.names = FALSE) 

# weight_initial, site
tuk <- TukeyHSD(wi_anova_site, "site_number")
tuk_df <- broom::tidy(tuk) %>%
  filter(term == "site_number") %>%
  separate(contrast, into = c("Group1", "Group2"), sep = "-") %>%
  mutate(Significant = ifelse(adj.p.value < 0.05, "Yes", "No"))
...
view(tukey_table_with_cld)
#write.csv(tukey_table_with_cld, "../output/tukey_weight_initial_site_10012025.csv", row.names = FALSE) 
...
view(tukey_table_with_cld)
#write.csv(tukey_table_with_cld, "../output/tukey_weight_final_site_10012025.csv", row.names = FALSE) 

```


# NOT Normally distributed indices - Kruskal-Wallis + Dunn's Post hoc
## Are there differences between these values at the site and reporting area level?
```{r}
...
# all of the contaminant indices are NOT normally distributed and will get a KW box for them v reporting area separately
...
#write.csv(dunn_table_with_cld, "../output/dunn_sod_site_10022025.csv", row.names = FALSE) 
...
view(dunn_table_with_cld)
#write.csv(dunn_table_with_cld, "../output/dunn_ci1_site_10022025.csv", row.names = FALSE) 
...
view(dunn_table_with_cld)
#write.csv(dunn_table_with_cld, "../output/dunn_ci2_site_10022025.csv", row.names = FALSE) 
...
view(dunn_table_with_cld)
#write.csv(dunn_table_with_cld, "../output/dunn_shell_site_10022025.csv", row.names = FALSE) 

# shell, ra - Not Significant
...
view(dunn_table_with_cld)
#write.csv(dunn_table_with_cld, "../output/dunn_shell_ra_10022025.csv", row.names = FALSE) 

# ibr_bio - Not Significant
dunn_result <- dunnTest(ibr_biomarker ~ site_name, data = sample_ibr, method = "bh") 
dunn_df <- dunn_result$res %>%
  dplyr::select(Comparison, Z, P.unadj, P.adj) %>%
  tidyr::separate(Comparison, into = c("Group1", "Group2"), sep = " - ") %>%
  dplyr::mutate(Significant = ifelse(P.adj < 0.05, "Yes", "No"))
...
view(dunn_table_with_cld)
#write.csv(dunn_table_with_cld, "../output/dunn_ibr_bio_site_10022025.csv", row.names = FALSE) 

# ibr_combined (total) - Not Significant
...
view(dunn_table_with_cld)
#write.csv(dunn_table_with_cld, "../output/dunn_ibr_total_site_10022025.csv", row.names = FALSE) 

```

# Dunn's Test - reporting area
## chlordane, ddt, nutrient + total metal, low + high + total pah, pbde, pcb
```{r}
...
view(dunn_table_with_cld)
#write.csv(dunn_table_with_cld, "/Users/cmantegna/Documents/Github/WDFWmussels/output/dunn_chlordane_ra.csv", row.names = FALSE) 
...
view(dunn_table_with_cld)
#write.csv(dunn_table_with_cld, "/Users/cmantegna/Documents/Github/WDFWmussels/output/dunn_ddt_ra.csv", row.names = FALSE) 
...
view(dunn_table_with_cld)
#write.csv(dunn_table_with_cld, "/Users/cmantegna/Documents/Github/WDFWmussels/output/dunn_nutrient_metal_ra.csv", row.names = FALSE) 

# total metal - Not Significant
...
view(dunn_table_with_cld)
#write.csv(dunn_table_with_cld, "/Users/cmantegna/Documents/Github/WDFWmussels/output/dunn_total_metal_ra.csv", row.names = FALSE) 
...
view(dunn_table_with_cld)
#write.csv(dunn_table_with_cld, "/Users/cmantegna/Documents/Github/WDFWmussels/output/dunn_high_pah_ra.csv", row.names = FALSE) 
...
view(dunn_table_with_cld)
#write.csv(dunn_table_with_cld, "/Users/cmantegna/Documents/Github/WDFWmussels/output/dunn_low_pah_ra.csv", row.names = FALSE) 
...
view(dunn_table_with_cld)
#write.csv(dunn_table_with_cld, "/Users/cmantegna/Documents/Github/WDFWmussels/output/dunn_total_pah_ra.csv", row.names = FALSE) 
...
view(dunn_table_with_cld)
#write.csv(dunn_table_with_cld, "/Users/cmantegna/Documents/Github/WDFWmussels/output/dunn_pbde_ra.csv", row.names = FALSE) 
...
view(dunn_table_with_cld)
#write.csv(dunn_table_with_cld, "/Users/cmantegna/Documents/Github/WDFWmussels/output/dunn_pcb_ra.csv", row.names = FALSE) 
