...
# Significance
## Variance, p= 0.7359, we can use PERMANOVA for the stats
```{r}
...
metric_matrix <- as.matrix(data[, 7:10]) # pull out my metrics - confirm column numbers are still correct
...
dist_matrix_full <- vegdist(as.matrix(data[, c("p450", "sod", "ci_wet_volume", "shell_thickness", "ci_wet_shell_weight", "ci_pollution")]), na.rm= TRUE, method = "euclidean") # create distance matrix
