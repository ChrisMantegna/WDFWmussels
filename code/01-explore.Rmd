...
## take a look at the dataframes to ensure values look correct
...
## fix row names and remove rows that aren't in my data frame

```{r}

#this code isn't working. It is functionally correct, but the data frame still reflects the names i don't want.
rownames(alldata)[rownames(alldata) == "Broad Spit"] <- "Broad Spit  (Fisherman's Point)"
rownames(alldata)[rownames(alldata) == "Penrose Point State Park" ] <- "Pennrose Point State Park"
rownames(alldata)[rownames(alldata) == "Maristone Island Bridge" ] <- "Maristone Island"
rownames(alldata)[rownames(alldata) == "Suquamish, Stormwater Outfall" ] <- "Suquamish Stormwater Outfall"
...
shist <- ggplot(alldata, aes(x = SOD)) +
...
# shapiro wilkes test for normality, p450: Data is not normally distributed
