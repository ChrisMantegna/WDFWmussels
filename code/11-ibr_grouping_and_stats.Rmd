...
# Build analyte_vars by excluding both metrics and IDs
analyte_vars <- colnames(df6)[!(colnames(df6) %in% c(metrics_vars, id_columns))]
...
## Spearmans, contaminant concentrations & lat/ long
```{r}
...
write.csv(bio_moran_results, "/Users/cmantegna/Documents/Github/WDFWmussels/output/spatial_correlation_biomarkers_analytes.csv", row.names = FALSE)


# ibr - metals
ibr_sf <- st_as_sf(df6, coords = c("longitude", "latitude"), crs = 4326)

coords <- st_coordinates(ibr_sf)
nb <- knn2nb(knearneigh(coords, k = 4))   # adjust k if needed
lw <- nb2listw(nb, style = "W")

biom_vars <- c("p450", "sod", "shell", "ci1", "ci2", "ci3", "ibr1bio", "ibr1morph", "ibr1overall")

biom_moran_results <- lapply(biom_vars, function(var) {
  vals <- ibr_sf[[var]]
  if (sum(!is.na(vals)) >= 4 && length(unique(vals[!is.na(vals)])) > 1) {
    m <- moran.test(vals, lw)
    data.frame(
...
# Optional: Add significance stars
biom_moran_results <- biom_moran_results %>%
  mutate(
    sig = case_when(
      p_value <= 0.001 ~ "***",
      p_value <= 0.01 ~ "**",
...
# View results
print(biom_moran_results)
write.csv(biom_moran_results, "/Users/cmantegna/Documents/Github/WDFWmussels/output/spatial_correlation_biomarkers_metals.csv", row.names = FALSE)
...
model_lm <- lm(ibr1overall ~ zinc + cadmium+ lead + latitude + longitude, data = df6)
summary(model_lm)
