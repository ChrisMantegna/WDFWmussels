...
#df1_analytes<- read.csv("../data/cleaned/avg_ibr1_analytess.csv")

# define variables
site<- df1_analytes$site_name
...
for (metric in colnames(metrics_vars)) {
  for (analyte in colnames(analyte_vars)) {
    ...
    x_valid <- x[valid_idx]
    y_valid <- y[valid_idx]
    sites_valid <- site[valid_idx]
    
    # Run correlations
    spearman <- cor.test(x_valid, y_valid, method = "spearman")
    kendall <- cor.test(x_valid, y_valid, method = "kendall")
    
    # create dataframe of significant results that includes site names
    if (spearman$p.value <= alpha | kendall$p.value <= alpha) {
      temp_df <- data.frame(
        Site = unique(sites_valid),  # Or collapse if multiple
        Metric = metric,
        Analyte = analyte,
      ...
      results_list[[paste(metric, analyte, sep = "_")]] <- temp_df
    }
  }
}
