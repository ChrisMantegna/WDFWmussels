...
kruskal.test(cf ~ PAHgroup5, data = data)
kruskal.test(cf ~ PAHgroup6, data = data)
kruskal.test(cf ~ PAHgroup7, data = data)
kruskal.test(cf ~ PAHgroup8, data = data)

```

##cf- Kruskal-Wallac Multiple Comparisons (post hoc)
### no significant differences across site, reporting area or PAHgroup 8
...
mc_group8<- as.data.frame(kruskalmc(cf ~ PAHgroup8, data = data, method = "bonferroni"))
...
data$PAHgroup5 <- as.character(data$PAHgroup5)
...
anova_group5 <- aov(at ~ PAHgroup5, data = data)
...
data$PAHgroup7 <- as.character(data$PAHgroup7)
data$PAHgroup8 <- as.character(data$PAHgroup8)

anova_group6 <- aov(at ~ PAHgroup6, data = data)
summary(anova_group6)

anova_group7 <- aov(at ~ PAHgroup7, data = data)
summary(anova_group7)
...
# significant  differences between 15 pairwise comparisons. Table written to GH repo.
tukey_group7<- TukeyHSD(anova_group7)
ttable_group7<- tidy(tukey_group7)
sigttable_group7 <- ttable_group7 %>% filter(adj.p.value < 0.05)
write.csv(sigttable_group7, "/Users/cmantegna/Documents/WDFWmussels/output/tables/SignificantThicknessGROUP7.csv", row.names = FALSE)
