...
kruskal.test(sod ~ PAHgroup, data = asdata)
kruskal.test(sod ~ PAHgroup2, data = asdata)
...
# significance confirmed between low and veryhigh

mc_site<- as.data.frame(kruskalmc(sod ~ site_name, data = asdata, method = "bonferroni"))
mc_group3<- as.data.frame(kruskalmc(sod ~ PAHgroup3, data = asdata, method = "bonferroni")) 
...
# Correlation test - SOD and lmwPAH
```{r}
...
# Correlation test - SOD and PAH16
```{r}
...
# correlation= 0.1558 with a p-value of .0059. This is a significant weak correlation.

correlation_result <- cor.test(asdata$sod, asdata$sumPCB, method = "pearson")
print(correlation_result)
```

# Correlation test - SOD and mercury
```{r}
...
# Correlation test - SOD and arsenic
```{r}
...
# Correlation test - SOD and cadmium
```{r}
...
# Correlation test - SOD and copper
```{r}
