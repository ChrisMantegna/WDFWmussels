...
df<- read.csv("../data/all_with_imputations_and_identifiers.csv") # load data
...
df$site[df$site == "Blair Waterway #2"] <- "Blair Waterway Two"
df$site[df$site == "Comm Bay Skookum"] <- "Commencement Bay Skookum"
df$site[df$site == "Comm Bay, Dick Gilmur Launch"] <- "Commencement Bay, Dick Gilmur Launch"
df$site[df$site == "Comm Bay, Milwaukee Waterway"] <- "Commencement Bay, Milwaukee Waterway"
df$site[df$site == "Meyer's Point - Henderson Inlet"] <- "Meyer's Point, Henderson Inlet"
df$site[df$site == "Purdy - Dexters"] <- "Purdy, Dexters"
df$site[df$site == "S of Skunk Island"] <- "South of Skunk Island"
df$site[df$site == "Suquamish, Stormwater Outfall"] <- "Suquamish Stormwater Outfall"
df$analyte[df$analyte == "mercuryTotal"] <- "mercury"
df$analyte[df$analyte == "Zinc"] <- "zinc"

df <- df %>%
  mutate(across(where(is.character), str_trim))

#save the new df's if need be
