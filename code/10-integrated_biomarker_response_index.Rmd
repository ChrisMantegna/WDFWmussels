...
#### Reference site data (Penn Cove)
ref_df <- df %>% filter(site_name == "Penn Cove Reference")

# Get reference means & SDs
ref_ci_means <- colMeans(ref_df[, condition], na.rm = TRUE)
ref_ci_sds   <- apply(ref_df[, condition], 2, sd, na.rm = TRUE)

# Z-scores relative to reference site (preserving sign)
condition_z <- sweep(df[, condition], 2, ref_ci_means, "-")
condition_z <- sweep(condition_z, 2, ref_ci_sds, "/")

# Combine into final df with signed values
ibr_df <- bind_cols(ibr_df, condition_z)
ibr_df <- ibr_df %>%
 mutate(
...
#### Reference site data (HC Sites only)
ref_df <- df %>% filter(site_number %in% c(29, 30))
...
#### Reference site data (All three references)
ref_df <- df %>% filter(site_number %in% c(3, 29, 30))
...
# Plotting
Plot overall for each and take a look - left off here on 4.12.25
```{r}

df<- read.csv("../data/cleaned/complete_cleaned_ibr.csv")
