...
model <- lm(p450 ~ SumBDEs + SumDDTs + SumPAHs16 + SumPAHsHMW + SumPCBs2x17, data = alldata)
...
```
#anova for p450 analytes
```{r}
#anova

#model <- lm(p450 ~ acenaphthene + acenaphthylene + anthracene + 'C1-flourenes', data = alldata)

#summary(model)
p450analyteanova<- anova(model)
print(p450analyteanova)
