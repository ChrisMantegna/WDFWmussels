...
# LISA
## Building the individual data layers for QGIS output
```{r}

# id the columns one more time in the projection to be sure
metric_cols <- names(sf_proj)[6:24]
metric_cols

# create the basis for the geopackage
outdir <- "../output/spatial_stats"

for (m in metric_cols) {

  map_df <- sf_proj |>
    select(site_number, site_name, reporting_area, all_of(m)) |>
    left_join(
      lisa_long |>
        filter(metric == m) |>
        select(site_number, local_i, p_value, p_adj_bh, quad, sig, sig_bh),
      by = "site_number"
    ) |>
    mutate(
      cluster_raw = if_else(sig, quad, "NotSig"),
      cluster_bh  = if_else(sig_bh, quad, "NotSig")
    )

  st_write(
    map_df,
    dsn = file.path(outdir, paste0("lisa_", m, ".gpkg")),
...
# Building a single geopackage with all layers for QGIS output
```{r}

# create the basis for the geopackage
outdir <- "../output/spatial_stats"

gpkg_path <- file.path(outdir, "lisa_all_layers.gpkg")
...
gpkg_path <- file.path(outdir, "global_morans_metrics.gpkg")
if (file.exists(gpkg_path)) file.remove(gpkg_path)

metrics <- global_sig_bh$metric |> as.character()

for (m in metrics) {

  map_df <- sf_proj |>
    dplyr::select(site_number, site_name, reporting_area, dplyr::all_of(m))

  sf::st_write(
    map_df,
    dsn    = gpkg_path,
    layer  = paste0("global_", m),
    quiet  = TRUE
  )
}
...
# Creating a geopackage of my complete data with identifiers and metrics
```{r}

site_data_clean <- site_data %>%
  mutate(
    # columns 1–2: lat / lon → numeric, 4 decimal places
    across(1:2, ~ round(as.numeric(.x), 4)),

    # columns 3–4: integers
    across(3:4, ~ as.integer(.x)),

    # column 5: site_name → leave as character
    across(5, as.character),

    # columns 6–24: numeric, 4 decimal places
    across(6:24, ~ round(as.numeric(.x), 4))
  )

site_data_sf <- st_as_sf(
  site_data_clean,
  coords = c(names(site_data_clean)[2], names(site_data_clean)[1]),
  crs = 4326,
  remove = FALSE
)
...
# Creating GeoPackage with adjustments for varying data in significant moran's metrics
```{r}

# define detected v percentile groups
vars_pct <- c("ddt_index", "pah_hmw_index", "pbde_index", "pcb_index", "total_con_index")
vars_det <- c("chlordane_index", "hch_index", "pesticide_index")

# define column/ data format
site_data_map <- site_data %>%
  mutate(
    # Make sure target columns are numeric (won't hurt if they already are)
    across(all_of(c(vars_pct, vars_det)), ~ as.numeric(.x))
  ) %>%
  mutate(
    # Percentile rank fields (0–100)
    across(all_of(vars_pct), ~ dplyr::percent_rank(.x) * 100, .names = "{.col}_pct"),

    # Detection fields: 0 = not detected, 1 = detected
    across(all_of(vars_det), ~ ifelse(is.na(.x) | .x <= 0, 0L, 1L), .names = "{.col}_det")
  )
...
# write geopackage
out_gpkg <- "../output/spatial_stats/moran_map_ready.gpkg"
