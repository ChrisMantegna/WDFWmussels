...
# Spatial Analysis
## Global Moran's I - only Local worked here - adjust the code later
```{r}
...
## Global - one metric at a time
```{r}

# p450
site_data <- site_sf %>% filter(!is.na(p450))
summary(site_data$p450)
coords <- st_coordinates(site_data)
library(spdep)
nb <- knn2nb(knearneigh(coords, k = 4))
lw <- nb2listw(nb, style = "W")
x_scaled <- scale(site_data$p450)[, 1]
...
```{r}
#p450
# 1. Filter your spatial data to rows where ibr_combined is not NA
site_data <- site_sf %>% filter(!is.na(p450))

# 2. Check that it has more than a few rows and some variance
summary(site_data$p450)

# 3. Get coordinates from the filtered sf object
coords <- st_coordinates(site_data)

# 4. Build neighbor list and weights
library(spdep)
nb <- knn2nb(knearneigh(coords, k = 4))
lw <- nb2listw(nb, style = "W")

# 5. Scale and run Moran’s I
x_scaled <- scale(site_data$p450)[, 1]
moran.test(x_scaled, lw)
```
