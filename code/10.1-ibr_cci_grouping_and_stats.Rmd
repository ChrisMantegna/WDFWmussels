...
k_optimal <- 6  # Choose based on elbow/silhouette/gap methods

# Run k-means
km <- kmeans(geo_data, centers = k_optimal, nstart = 25)

# Add cluster assignments to dataset
all_data$cluster6 <- as.factor(km$cluster)

ggplot(all_data, aes(x = longitude, y = latitude, color = cluster6)) +
  geom_point(size = 3) +
...
dist_matrix <- vegdist(df_anosim %>%
                         select(log2_p450, log2_sod, log2_shell, log2_ci1, log2_ci2, log2_ci3, ibr_combined) %>%
                         scale(), method = "euclidean")

# Run ANOSIM
anosim_results <- anosim(dist_matrix, grouping = df_anosim$cluster2)
