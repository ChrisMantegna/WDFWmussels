...
df<- read.csv("../data/cleaned/all_samples_all_groups.csv")
...
# Run Two-Way PERMANOVA
permanova <- adonis2(vegdist(scale(df$sod), method = "euclidean") ~ site_name * p16, 
...
# Run post-hoc pairwise tests
pairwise_geo <- pairwise.perm.manova(vegdist(scale(df$sod), method = "euclidean"), df$site_name, nperm = 999)
pairwise_contam <- pairwise.perm.manova(vegdist(scale(df$sod), method = "euclidean"), df$p16, nperm = 999)
pairwise_interaction <- pairwise.perm.manova(vegdist(scale(df$sod), method = "euclidean"), interaction(df$site_name, df$p16), nperm = 999)
