...
# read- in data if using a different table than the block before

# Step 1: Extract contaminant index matrix
contaminants <- site_data[, c("total_metal", "total_pah", "pbde", "pcb", "pesticide")]

# Step 2: Scale the data (z-score standardization)
contaminants_scaled <- scale(contaminants)

# Step 3: Run PCA
pca_result <- prcomp(contaminants_scaled, center = TRUE, scale. = TRUE)

# Step 4: Extract PC1 scores as the PCA-based mixture index
site_data$mixture_index_pca <- pca_result$x[, 1]

# Optional: View how much variance is explained by PC1
...
### not sure if this is still an accurate code block - check after work is done
```{r}
