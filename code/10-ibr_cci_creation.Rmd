...
metal_index <- metals_ai %>%
  rowwise() %>%
  mutate(
    heavy_metal_index = sum(c_across(starts_with("ai_z_")[heavy_metals %in% sub("z_", "", names(.)[grepl("ai_z_", names(.))])]), na.rm = TRUE),
    nutrient_metal_index = sum(c_across(starts_with("ai_z_")[nutrient_metals %in% sub("z_", "", names(.)[grepl("ai_z_", names(.))])]), na.rm = TRUE),
...
#write.csv(metal_index, "/Users/cmantegna/Documents/Github/WDFWmussels/data/index_df/metal_index.csv", row.names = FALSE)
...
baseline_sites <- c("Penn Cove Baseline", "Broad Spit Baseline", "Hood Canal Holly Baseline")
ref_metal_df <- metals_wide %>% filter(site_name %in% baseline_sites)

metal_vars <- c("Zinc", "Copper", "Lead", "Arsenic", "Mercury", "Cadmium") # how to call column numbers instead of names?

ref_metal_means <- colMeans(ref_metal_df[, metal_vars], na.rm = TRUE)
ref_metal_sds   <- apply(ref_metal_df[, metal_vars], 2, sd, na.rm = TRUE)

metal_z <- sweep(metals_wide[, metal_vars], 2, ref_metal_means, "-")
metal_z <- sweep(metal_z, 2, ref_metal_sds, "/")

contaminant_index_df <- metals_wide %>%
  select(site_name, latitude, longitude) %>%  # include other metadata as needed
  bind_cols(metal_z) %>%
  mutate(cci_metals = rowMeans(metal_z, na.rm = TRUE))
...
head(metrics)
# created/ saved 5-7-2025
#write.csv(metrics, "/Users/cmantegna/Documents/Github/WDFWmussels/data/index_df/metrics_site_level.csv", row.names = FALSE)
