...
# subtract reference means and take absolute value
df_ai <- df_z %>%
  mutate(across(
    starts_with("z_"),
    .fns = function(z, nm) abs(z - ref_means[[nm]]),
    .names = "ai_{.col}",
    .names_sep = ""
  ))
...
df_ibr <- df_ai %>%
  rowwise() %>%
  mutate(IBRv2i = sum(c_across(starts_with("ai_")), na.rm = TRUE)) %>%
  ungroup()

```


## IBRs - biomarker, morphometric, combined
```{r}
