...
## log2 transform the metrics - NOT used in 2/3 analysis ouptputs (3.0 and 3.1), IS used in 1/3 analysis outputs (3.2)
```{r}
...
## z-score standardization, absolute sd (AI) & IBR scoring
```{r}
...
# ---- pick transformed columns to process ----
...
# ---- add centered (_s), z of centered (_zs), and AI on centered (ai_*_s) ----
# 1) center *_t -> *_s (keeps raw & *_t untouched)
metrics_transformed <- metrics_transformed %>%
  mutate(across(all_of(t_cols),
                ~ .x - centers[cur_column()],
                .names = "{.col}_s")) %>%
  # rename to base_s regardless of style:
  rename_with(~ sub("_t_s$", "_s", .x), ends_with("_t_s")) %>%
  rename_with(~ sub("^t_(.*)_s$", "\\1_s", .x), matches("^t_.*_s$"))

# 2) z-scores of centered for IBR metrics only
ibr_t_cols <- resolve_t(metrics_transformed, metrics_for_ibr)         # ensure alignment
...
metrics_transformed <- metrics_transformed %>%
  mutate(across(all_of(s_cols_ibr),
                ~ as.numeric(scale(.x)),
                .names = "{.col}_zs"))                                # e.g., p450_s_zs

# 3) absolute deviation (AI) from reference on centered scale
metrics_transformed <- metrics_transformed %>%
  mutate(across(all_of(s_cols_ibr),
                ~ abs(.x),
                .names = "ai_{.col}"))                                # e.g., ai_p450_s
...
# Example save:
# write.csv(metrics_transformed, "metrics_transformed_with_scaling.csv", row.names = FALSE)
