...
# ---------------------------
# STEP 1 — TRANSFORM (sample-level only)
# ---------------------------
# Keep only columns we need, then add t_* = log2(pmax(x, eps))
sample_level_t <- df %>%
...
# ---------------------------
# STEP 2 — SCALE TO REFERENCE (run when your reference samples are present)
#         Produces tc_*: deviations from reference on the log scale
# ---------------------------
# 2a) rows for reference site
reference_samples <- sample_level_t %>%
  filter(.data[[site_col]] == ref_site)

if (nrow(reference_samples) == 0) {
  stop(sprintf("No rows found for ref_site = '%s' in column '%s'.", ref_site, site_col))
}

# 2b) compute reference centers on t-scale
#     - geometric-set: mean of t_* (equivalent to geometric mean on raw scale)
#     - arithmetic-set: mean on raw scale, then log2 (keeps parity with arithmetic intent)
arith_metrics <- setdiff(all_metrics, geom_center_metrics)

# geometric centers
centers_geom_t <- if (length(geom_center_metrics) > 0) {
  reference_samples %>%
    summarize(across(all_of(t_name(geom_center_metrics)), ~ mean(.x, na.rm = TRUE))) %>%
    pivot_longer(everything(), names_to = "metric_t", values_to = "center_t") %>%
    mutate(metric = str_remove(metric_t, "^t_")) %>%
    select(metric, center_t)
} else {
  tibble(metric = character(0), center_t = numeric(0))
}

# arithmetic centers: average on raw, then convert to t-scale
# recover raw from t_ via 2^t
centers_arith_t <- if (length(arith_metrics) > 0) {
  reference_samples %>%
    transmute(across(all_of(t_name(arith_metrics)), ~ 2^.x)) %>%
    summarize(across(everything(), ~ mean(.x, na.rm = TRUE))) %>%
    pivot_longer(everything(), names_to = "raw_t_metric", values_to = "center_raw_mean") %>%
    mutate(metric   = str_remove(raw_t_metric, "^t_"),
...
centers_t <- bind_rows(centers_geom_t, centers_arith_t)

# 2c) Join centers back to all samples and compute tc_* = t_* - center_t
sample_level_tc <- sample_level_t %>%
  pivot_longer(cols = all_of(t_name(all_metrics)),
               names_to = "metric_t", values_to = "t_value") %>%
  mutate(metric = str_remove(metric_t, "^t_")) %>%
  left_join(centers_t, by = "metric") %>%
  mutate(tc_value = t_value - center_t,
         tc_col   = tc_name(metric)) %>%
  select(-metric_t) %>%
  pivot_wider(names_from = c(metric, tc_col),
              values_from = c(t_value, tc_value),
              names_sep = "") %>%
  relocate(starts_with("t_"),  .after = last_col()) %>%
  relocate(starts_with("tc_"), .after = last_col())
...
# ---------------------------
# STEP 3 — (LATER) BUILD INDICES, THEN COLLAPSE TO SITE
# ---------------------------
# Example: simple additive index from tc_* columns (edit as needed)
 tc_cols <- names(sample_level_tc)[startsWith(names(sample_level_tc), "tc_")]
 sample_level_index <- sample_level_tc %>%
   rowwise() %>%
   mutate(IBR_like_index = sum(c_across(all_of(tc_cols)), na.rm = TRUE)) %>%
   ungroup()

# Collapse to one value per site (mean of the sample-level index, for example)
 site_level_index <- sample_level_index %>%
   group_by(.data[[site_col]]) %>%
   summarize(IBR_like_index = mean(IBR_like_index, na.rm = TRUE),
             n_samples = n(), .groups = "drop")

 head(site_level_index)
# write.csv(site_level_index, "site_level_index.csv", row.names = FALSE)
...
## log2 transform the metrics - site-level used in analysis (3.2)
```{r}
