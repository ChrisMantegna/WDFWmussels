...
#site_data<- read.csv("../data/start_here/site_level_no_indices.csv")
sample_data<- read.csv("../data/cleaned/cleaned_sample_level.csv")
metal<- read.csv("../data/interim_df/metal_converted_for_index_creation.csv")
...
sample_data$sample_id<- as.factor(sample_data$sample_id)
sample_data$p450<- as.numeric(sample_data$p450)

metal$arsenic<- as.numeric(metal$arsenic)
metal$cadmium<- as.numeric(metal$cadmium)
metal$copper<- as.numeric(metal$copper)
metal$lead<- as.numeric(metal$lead)
metal$mercury<- as.numeric(metal$mercury)
metal$zinc<- as.numeric(metal$zinc)

#summary(sample_data)

```

# IBRv2i Index creation
## log2 transform the metrics - sample-level
```{r}
...
id_cols <- c("sample_id", "site_number","site_name", "reporting_area", "latitude", "longitude")

# metrics for the IBR 
metric_cols <- c("p450","sod", "shell", "weight_initial", "weight_change", "weight_final", "length","height","width")

# metrics not for the IBR
...
t_name  <- function(x) paste0("t_", x)   # transformed
tc_name <- function(x) paste0("tc_", x)  # centered vs reference

# transform
sample_level_t <- df %>%
  select(any_of(unique(c(id_cols, site_col, all_metrics)))) %>%
  mutate(across(all_of(all_metrics), ~ log2(pmax(.x, eps)), .names = "t_{.col}")) %>%
  relocate(any_of(id_cols), .before = 1) %>%
  relocate(starts_with("t_"), .after = last_col())

# view & save
head(sample_level_t)
#write.csv(sample_level_t, "../data/interim_df/sample_level_transformation_09292025.csv", row.names = FALSE) 

```

## scale to Penn Cove Reference site
```{r}

# scale
reference_samples <- sample_level_t %>%
  dplyr::filter(.data[[site_col]] == ref_site)

if (nrow(reference_samples) == 0) {
  stop(sprintf("No rows found for ref_site = '%s' in column '%s'.", ref_site, site_col))
}

# calculate reference centers on the t-scale
arith_metrics <- setdiff(all_metrics, geom_center_metrics)

# geometric: mean of t_<metric>
centers_geom_t <- if (length(geom_center_metrics) > 0) {
  reference_samples %>%
    dplyr::summarize(dplyr::across(
      dplyr::all_of(paste0("t_", geom_center_metrics)),
      ~ mean(.x, na.rm = TRUE)
    )) %>%
    tidyr::pivot_longer(dplyr::everything(),
      names_to = "metric_t", values_to = "center_t"
    ) %>%
    dplyr::mutate(metric = stringr::str_remove(metric_t, "^t_")) %>%
    dplyr::select(metric, center_t)
} else {
  tibble::tibble(metric = character(0), center_t = numeric(0))
}

# arithmetic: mean on *raw*, then log2
centers_arith_t <- if (length(arith_metrics) > 0) {
  # recover raw from t_ via 2^t (ref rows only)
  reference_samples %>%
    dplyr::transmute(dplyr::across(
      dplyr::all_of(paste0("t_", arith_metrics)),
      ~ 2^.x
    )) %>%
    dplyr::summarize(dplyr::across(dplyr::everything(), ~ mean(.x, na.rm = TRUE))) %>%
    tidyr::pivot_longer(dplyr::everything(),
      names_to = "raw_t_metric", values_to = "center_raw_mean"
    ) %>%
    dplyr::mutate(
      metric   = stringr::str_remove(raw_t_metric, "^t_"),
      center_t = log2(pmax(center_raw_mean, eps))
    ) %>%
    dplyr::select(metric, center_t)
} else {
  tibble::tibble(metric = character(0), center_t = numeric(0))
...
centers_t <- dplyr::bind_rows(centers_geom_t, centers_arith_t)

# long format to compute & then compute tc_value
long_t <- sample_level_t %>%
  tidyr::pivot_longer(
    cols = dplyr::all_of(paste0("t_", all_metrics)),
    names_to = "metric_t",
    values_to = "t_value"
  ) %>%
  dplyr::mutate(metric = stringr::str_remove(metric_t, "^t_")) %>%
  dplyr::left_join(centers_t, by = "metric") %>%
  dplyr::mutate(tc_value = t_value - center_t)

# wide format
join_keys <- if (site_col %in% id_cols) id_cols else unique(c(id_cols, site_col))

wide_t <- long_t %>%
  dplyr::select(dplyr::all_of(join_keys), metric, t_value) %>%
  tidyr::pivot_wider(
    id_cols    = dplyr::all_of(join_keys),
    names_from = metric,
    values_from = t_value,
    names_glue = "t_{metric}"
  )

wide_tc <- long_t %>%
  dplyr::select(dplyr::all_of(join_keys), metric, tc_value) %>%
  tidyr::pivot_wider(
    id_cols    = dplyr::all_of(join_keys),
    names_from = metric,
    values_from = tc_value,
    names_glue = "tc_{metric}"
  )

# final table in current format
sample_level_tc <- wide_t %>%
  dplyr::left_join(wide_tc, by = join_keys) %>%
  # optional: tidy column order
  dplyr::relocate(dplyr::starts_with("t_"),  .after = dplyr::last_col()) %>%
  dplyr::relocate(dplyr::starts_with("tc_"), .after = dplyr::last_col())

# view & save
head(sample_level_tc)
#write.csv(sample_level_tc, "../data/interim_df/sample_level_scaling_09292025.csv", row.names = FALSE) 

```

##  adding absolute deviation from the reference (ai) and creating the ibr score
```{r}

# where to find the columns to create ai
tc_cols <- grep("^tc_", names(sample_level_tc), value = TRUE)

# ID biomarker and morphometric metrics
biomarker_vars <- c("tc_p450", "tc_sod")
morphometric_vars <- c("tc_shell", "tc_weight_change", "tc_length", "tc_height", "tc_width")
condition_vars<- c("tc_weight_initial","tc_weight_final", "tc_ci1", "tc_ci2", "tc_ci3")

# calculate ai
sample_level_ai <- sample_level_tc %>%
  dplyr::mutate(dplyr::across(
    dplyr::all_of(tc_cols),
    ~ abs(.x),
    .names = "ai_{.col}"
  ))

# identify ai column groups
ai_biomarkers<- paste0("ai_", biomarker_vars)
ai_morphos<- paste0("ai_", morphometric_vars)
ai_conditions<- paste0("ai_", condition_vars)
ai_cols<- c(ai_biomarkers, ai_morphos)

# C) Build indices
sample_level_ibr <- sample_level_ai %>%
  dplyr::rowwise() %>%
  dplyr::mutate(
    ibr_biomarker= sum(c_across(all_of(ai_biomarkers)), na.rm = TRUE),
    ibr_morphometric = sum(c_across(all_of(ai_morphos)), na.rm = TRUE),
    ibr_total= sum(c_across(all_of(ai_cols)), na.rm = TRUE)
  ) %>%
  dplyr::ungroup()

# D) Review
head(sample_level_ibr)
#write.csv(sample_level_ibr, "../data/interim_df/sample_level_ibr_09292025.csv", row.names = FALSE)

```

## create site-level ibr from the sample-level ibr
```{r}

sample_level_ibr<- read.csv("../data/interim_df/sample_level_ibr_09292025.csv")

site_level_ibr <- sample_level_ibr %>%
  dplyr::group_by(site_name) %>%
  dplyr::summarize(
    ibr_bio     = mean(ibr_biomarker, na.rm = TRUE),
    ibr_morph   = mean(ibr_morphometric, na.rm = TRUE),
    ibr_combined= mean(ibr_total, na.rm = TRUE),
    # averaged scaled values
    across(starts_with("tc_"), ~mean(.x, na.rm = TRUE), .names = "avg_{.col}"),
    n_samples   = dplyr::n(),
    .groups = "drop"
  )
...
# Review & save
head(site_level_ibr)
#write.csv(site_level_ibr, "../data/site_level_ibr_and_metrics_10012025.csv", row.names = FALSE)

```

# Chemical Contaminant Indices 
## created based on chemical class
### chlordane
```{r}
...
total_metals <- c(heavy_metals, nutrient_metals)

# z-score for each analyte (site-level is not possible based on single site values)
metals_z <- metal %>%
  mutate(across(all_of(total_metals), scale, .names = "z_{.col}"))
...
# view & save
view(metal_index)
#write.csv(metal_index, "../data/interim_df/metal_index_10042025.csv", row.names = FALSE)
...
# Total Contamiant Load Index
```{r}

colnames(site_data)
# leaving out the metal and pah separate indices, only used total metal and total pah
